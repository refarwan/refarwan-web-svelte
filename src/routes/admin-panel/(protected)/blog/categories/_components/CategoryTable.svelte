    ...
    <div class="flex flex-col gap-3 md:gap-0">
        ...
        {:else}
            {#each items as item (item.id)}
                <div
                    class="grid grid-cols-[1fr_auto] gap-x-3 gap-y-2 rounded-xl border border-gray-200 bg-white p-4 transition-all duration-200 hover:bg-gray-50/60 md:grid-cols-[1fr_240px_90px] md:items-center md:gap-4 md:rounded-none md:border-x md:border-t-0 md:border-b md:border-gray-200 md:px-4 md:py-3 md:last:rounded-b-xl"
                >
                    ...
                    <div
                        class="col-span-2 flex items-center justify-end gap-2 pt-2 md:order-3 md:col-auto md:justify-center md:pt-0"
                    >
                        <button
                            type="button"
                            onclick={() => onEdit(item)}
