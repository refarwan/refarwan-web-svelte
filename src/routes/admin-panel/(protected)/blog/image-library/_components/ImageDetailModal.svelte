<script lang="ts">
    import { Check as CheckIcon, Copy as CopyIcon } from "lucide-svelte/icons";
...
<div
    class="flex h-auto max-h-[90vh] w-[94vw] max-w-89.5 flex-col overflow-hidden rounded-xl bg-white shadow-xl sm:max-w-125 md:max-w-150"
    role="dialog"
    aria-modal="true"
>
    ...
    >
        ...
        >
            {#each variants as v (v.key)}
                ...
                <button
                    ...
                    class={`flex shrink-0 cursor-pointer items-center justify-center gap-1 rounded-full px-3 py-1 text-[12px] transition-colors ${
                        isActive
                            ? "bg-theme-600 font-medium text-white shadow-2xs"
        ...
        <div
            class="relative flex aspect-video w-full shrink-0 items-center justify-center overflow-hidden rounded-lg bg-black shadow-sm"
        >
            <img
