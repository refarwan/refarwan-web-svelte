<script lang="ts">
    import { CheckIcon, SlidersHorizontalIcon } from "lucide-svelte/icons";
    import type { AdminOptionItem } from "$lib/types";
    ...
    let tempSelectedIds = $state<string[]>([...activeCategoryIds]);
...
<div class="relative" bind:this={dropdownRef}>
    <button
        ...
        class={`inline-flex cursor-pointer items-center gap-2 rounded-lg border px-3.5 py-2.5 text-sm font-medium transition-all ${
            isOpen || activeCategoryIds.length > 0
                ? "border-theme-600 bg-theme-50/50 text-theme-700 ring-1 ring-theme-500/20"
                : "border-gray-300 bg-white text-gray-700 hover:bg-gray-50"
        }`}
    >
        <SlidersHorizontalIcon class="h-4 w-4 text-gray-600" />
        <span>Filter</span>
    ...
    {#if isOpen}
        <div
            class="absolute right-0 z-40 mt-2 w-64 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xl md:right-auto md:left-0"
        >
            ...
            <div class="max-h-56 scrollbar-thin overflow-y-auto py-1.5">
                ...
                {:else}
                    {#each categories as category (category.id)}
                        ...
                        >
                            <div
                                class={`flex h-4.5 w-4.5 shrink-0 items-center justify-center rounded-sm transition-all ${
                                    isChecked
                                        ? "border border-theme-600 bg-theme-600 text-white"
                                        : "border-[1.5px] border-gray-300 bg-white hover:border-gray-400"
                                }`}
                            >
                                {#if isChecked}
                                    <CheckIcon class="h-3 w-3 stroke-3" />
                                {/if}
                            </div>
            ...
            <div class="flex items-center justify-end gap-2 bg-gray-50/60 px-4 py-2.5">
                ...
                <button
                    type="button"
                    onclick={applyFilter}
                    class="cursor-pointer rounded-md bg-theme-600 px-3.5 py-1.5 text-xs font-medium text-white shadow-2xs transition-colors hover:bg-theme-700"
                >
                    {t.apply}
                </button>
            </div>
