<script lang="ts">
	import ThumbnailUploadMode from './ThumbnailUploadMode.svelte';
...
<div class="space-y-2">
	...
	<div class="flex items-center rounded-md bg-gray-200 p-0.5">
		<button
			...
			class={`flex-1 cursor-pointer rounded-[5px] px-3 py-1.5 text-center text-xs font-medium transition-all ${
				mode === 'upload'
		...
		<button
			...
			class={`flex-1 cursor-pointer rounded-[5px] px-3 py-1.5 text-center text-xs font-medium transition-all ${
				mode === 'video' ? 'bg-white text-gray-900 shadow-2xs' : 'text-gray-500 hover:text-gray-900'
			}`}
		>
