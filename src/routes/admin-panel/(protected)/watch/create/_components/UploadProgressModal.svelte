<script lang="ts">
    import { LoaderCircle, CloudUploadIcon } from "lucide-svelte/icons";
    interface Props {
...
<div class="w-full max-w-md space-y-5 rounded-2xl border border-gray-100 bg-white p-6 shadow-2xl">
    <div class="flex items-start gap-3.5">
        <div
            class="flex size-12 shrink-0 items-center justify-center rounded-xl bg-theme-50 text-theme-600"
        >
            <CloudUploadIcon class="size-6 animate-pulse" />
        </div>
    ...
    <div class="flex justify-end pt-1">
        <button
            type="button"
            onclick={onCancel}
            class="cursor-pointer rounded-lg border border-gray-200 bg-white px-4 py-2 text-xs font-medium text-gray-700 shadow-2xs transition-colors hover:bg-gray-50 hover:text-red-600"
        >
