<script lang="ts">
    ...
    const editHref = $derived(
        resolve("/admin-panel/(protected)/watch/videos/edit/[id]", { id: video.id })
    );
</script>
...
<div
    class={`grid grid-cols-[68px_auto] gap-x-3 gap-y-1 rounded-xl border border-gray-200 bg-white px-3 pt-4 pb-3 transition-all duration-300 lg:grid-cols-[56px_1fr_245px_120px_60px_110px] lg:items-center lg:gap-4 lg:rounded-none lg:border-t-0 lg:last:rounded-b-xl xl:grid-cols-[64px_1fr_245px_120px_60px_110px] ${
        isDeleting
            ? "pointer-events-none scale-99 border-red-200 bg-red-50/70 opacity-40"
            : "hover:bg-gray-50/60"
    }`}
>
    ...
    <div class="flex items-center text-[13px] text-gray-500 lg:order-5 lg:text-gray-900">
        <EyeIcon class="h-3.5 w-3.5 lg:hidden" />
        <span class="ml-1">
    ...
    <div class="flex w-full justify-end gap-3 lg:order-6">
        ...
        >
            {#if isArchiving}
                <Loader2Icon class="h-3.5 w-3.5 animate-spin text-gray-500" />
            {:else if isArchived}
                <ArchiveRestoreIcon class="h-3.5 w-3.5" />
            {:else}
        ...
        >
            {#if isDeleting}
                <Loader2Icon class="h-3.5 w-3.5 animate-spin text-red-500" />
            {:else}
