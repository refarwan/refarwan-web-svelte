<script lang="ts">
    ...
    let activeCodes = $state(untrack(() => [...initialActiveCodes]));
...
<div class="relative w-full max-w-105 rounded-xl border border-gray-200 bg-white">
    ...
    <div class="max-h-72 space-y-1 overflow-y-auto px-4 py-2">
        {#each filteredLanguages as item (item.code)}
            ...
            >
                ...
                {#if item.code === "en"}
                    <span
                        class="rounded-full bg-[#edf7ed] px-2.5 py-0.5 text-xs font-medium text-[#268c40]"
                    >
                        {defaultLabel}
                    </span>
                {:else if active}
