<script lang="ts">
    ...
    const SHADE_KEYS: (keyof ThemeShades)[] = [
        "50",
        "100",
        "200",
        "300",
    ...
    let previewShades = $state<ThemeShades>(
        untrack(() => generateColorShades(initialColor) ?? DEFAULT_THEME_SHADES)
    );
...
<div class="flex w-full max-w-md flex-col gap-4 rounded-xl border border-gray-200 bg-white p-5">
        ...
        <div class="grid grid-cols-11 gap-1">
            {#each SHADE_KEYS as key (key)}
                ...
                <div class="flex flex-col items-center gap-1" title={`${key}: ${hex}`}>
                    <div
                        class="h-7 w-full rounded-sm border border-black/10"
    ...
    <div class="flex justify-end gap-2 border-t border-gray-100 pt-3">
        ...
        <button
            type="button"
            onclick={handleApply}
            class="cursor-pointer rounded-md bg-theme-600 px-4 py-2 text-xs font-medium text-white hover:bg-theme-700"
        >
            {applyLabel}
        </button>
    </div>
