...
class VideoListState {
    list = $state<ListResponse<VideoItem[]> | null>(null);
    ...
    async fetchDetail(id: string): Promise<VideoDetail | null> {
        try {
            const res = await authorizedHttp.get<DataResponse<VideoDetail>>(`/video/${id}`);
    ...
    async archive(id: string): Promise<void> {
        await authorizedHttp.patch(`/video/${id}/archive`);
        await this.refetch();
    }

    async unarchive(id: string): Promise<void> {
        await authorizedHttp.patch(`/video/${id}/unarchive`);
        await this.refetch();
    }

    async remove(id: string): Promise<void> {
        await authorizedHttp.delete(`/video/${id}`);
        await this.refetch();
    }
}
