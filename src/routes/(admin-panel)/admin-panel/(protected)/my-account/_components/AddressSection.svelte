<script lang="ts">
    ...
    let regencies = $state<AreaItem[]>(untrack(() => initialRegencies));
    let districts = $state<AreaItem[]>(untrack(() => initialDistricts));
    let villages = $state<AreaItem[]>(untrack(() => initialVillages));
    ...
    const villageOptions = $derived(
        villages.map((item) => ({ value: item.code, label: item.name }))
    );
