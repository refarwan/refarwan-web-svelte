...
import type { ArticleCategoryDetail, ArticleCategoryItem } from "$lib/types/article-category";
...
class CategoryListState {
    list = $state<ListResponse<ArticleCategoryItem[]> | null>(null);
    loading = $state(true);
    ...
    private async fetchList(pageParam: string, search: string) {
        ...
        try {
            ...
            const res = await authorizedHttp.get<ListResponse<ArticleCategoryItem[]>>(
                `/article-category?${query.toString()}`
            );
            this.list = res.data;
        } catch {
            this.list = null;
    ...
    async fetchDetail(id: string): Promise<ArticleCategoryDetail | null> {
        try {
            const res = await authorizedHttp.get<DataResponse<ArticleCategoryDetail>>(
                `/article-category/${id}`
            );
    ...
    async checkSlug(slug: string, currentSlug?: string): Promise<string> {
        ...
        try {
            const res = await authorizedHttp.get<DataResponse<{ slug: string }>>(
                `/article-category/slug-check?${params.toString()}`
            );
    ...
    async create(payload: CategoryPayload): Promise<void> {
        await authorizedHttp.post("/article-category", payload);
        await this.refetch();
    }

    async update(id: string, payload: CategoryPayload): Promise<void> {
        await authorizedHttp.put(`/article-category/${id}`, payload);
        await this.refetch();
    }

    async remove(id: string): Promise<void> {
        await authorizedHttp.delete(`/article-category/${id}`);
        await this.refetch();
    }
}
