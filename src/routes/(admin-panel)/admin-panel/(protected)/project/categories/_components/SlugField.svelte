...
<div class="space-y-1.5">
    ...
    <div class="relative">
        <input
            ...
            class="w-full rounded-lg border border-gray-300 bg-white py-2.5 pr-9 pl-3.5 text-sm text-gray-900 transition-all focus:border-theme-500 focus:ring-1 focus:ring-theme-500 focus:outline-none disabled:cursor-not-allowed disabled:bg-gray-100 disabled:text-gray-400"
        />
        {#if isCheckingSlug}
            <div class="absolute top-1/2 right-3 -translate-y-1/2">
                <Icon icon="lucide:loader-circle" class="h-4 w-4 animate-spin text-gray-400" />
            </div>
        {/if}
    </div>
