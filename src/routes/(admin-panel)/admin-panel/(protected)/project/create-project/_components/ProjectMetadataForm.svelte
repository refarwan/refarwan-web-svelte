<script lang="ts">
    ...
    interface Props {
        ...
        content?: unknown;
        onContentChange?: (value: { json: unknown; html: string }) => void;
    }
...
<div class="w-full min-w-0 flex-1 space-y-5">
    <div class="space-y-1.5">
        <label class="block text-sm font-medium text-gray-700" for="project-title"
            >{t.titleLabel}</label
        >
        <input
            id="project-title"
    ...
    <div class="space-y-1.5">
        <label class="block text-sm font-medium text-gray-700" for="project-short-description"
        ...
        <textarea
            ...
            class="min-h-20 w-full resize-y rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm text-gray-900 transition-all placeholder:text-gray-400 focus:border-theme-500 focus:ring-1 focus:ring-theme-500 focus:outline-none"
        ></textarea>
    </div>
    ...
    <div class="space-y-1.5">
        ...
        {#key activeLangCode}
            <RichTextEditor t={editorT} initialContent={content} onChange={onContentChange} />
        {/key}
    </div>
</div>
