			...
			>
				...
				{#if item.type === 'errorAlert'}
					<div class="flex items-center gap-3">
						<CircleX class="animate-popup-icon h-8 w-8 shrink-0 text-red-600" />
						<h3 class="text-lg font-bold text-gray-900">{item.title ?? 'Error'}</h3>
					</div>
				{:else if item.type === 'confirm'}
					<div class="flex items-center gap-3">
						<CircleHelp class="animate-popup-icon h-8 w-8 shrink-0 text-sky-600" />
						<h3 class="text-lg font-bold text-gray-900">{item.title ?? 'Confirm Action'}</h3>
					</div>
				{:else if item.type === 'successAlert'}
					<div class="flex items-center gap-3">
						<CircleCheckBig class="animate-popup-icon h-8 w-8 shrink-0 text-emerald-600" />
